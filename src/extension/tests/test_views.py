...
from .. import const, commands, models
...
class AuthViewsTest(TestCase):
    ...
    def test_post_success(self):
        ...
        # 実行
        response = self.client.post(self.url, {"user_id": 100000000000000000})
    ...
    def test_post_exists_token_expired(self):
        ...
        models.DiscordAuthenticationToken.objects.create(
            user_id=100000000000000000, generate_date_time=timezone.now() + timezone.timedelta(minutes=-61)
        )
    ...
    def test_post_exists_token_expired_set_ex_value(self):
        ...
        models.DiscordAuthenticationToken.objects.create(
            user_id=100000000000000000, generate_date_time=timezone.now() + timezone.timedelta(minutes=-6)
        )
